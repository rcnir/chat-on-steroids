...
function createLoader(options = {}) {
  ...
  async function handleBridge(context) {
    const { req, res, route, origin, readBody, json, tooLarge, conversationId } = context || {};
    ...
    const cleanConversation = typeof conversationId === 'function' ? conversationId(body.conversationId) : null;
    if (!cleanConversation) return badRequest(json, res, origin, 'bad_conversation_id');

    if (route === '/browser/next') {
      const command = control.collectBrowserCommand(cleanConversation);
    ...
    const accepted = control.settleBrowserCommand(cleanConversation, id, body.result);
    if (!accepted) {
