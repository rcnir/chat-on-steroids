...
const ACTIVITY_POLL = `    // <RC-BROWSER-CONTROL:poll>\n    // Fire-and-forget: normal ChatGPT activity must never wait on browser automation.\n    // With no registered driver Task 1 collects nothing, preserving retry-safe timeouts.\n    if (ownsDocument(source) && result.ok && __rcnirBrowserControlTransport) {\n      void __rcnirBrowserControlTransport.poll(message.conversationId);\n    }\n    // </RC-BROWSER-CONTROL:poll>\n`;
