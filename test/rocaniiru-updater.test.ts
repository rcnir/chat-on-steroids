...
it('accepts Chrome extension host-permission fetches without weakening the updater CSRF boundary', () => {
  const config = { extensionOrigin: 'chrome-extension://lcggilneomlkgcaeniefpkadadpfbgfn', port: 8768 };
  expect(updaterRequestAllowed(config, { origin: config.extensionOrigin })).toBe(true);
  expect(updaterRequestAllowed(config, {
    host: '127.0.0.1:8768',
    'sec-fetch-site': 'none',
    'sec-fetch-mode': 'cors',
...
it('selects the independent addon for verified releases and never uses the legacy fallback', () => {
  ...
  for (const version of ['2.0.6', '2.0.7', '2.0.8', '2.0.9', '2.1.11']) {
    expect(patchRecipe(config, version)).toMatchObject({ kind: 'task-box-addon', featureVersion: '1.0.8' });
  }
